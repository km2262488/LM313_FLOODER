        ...
        while retry < MAX_RETRY and not success and not stop_flag:
            try:
                ...
                with urllib.request.urlopen(req, timeout=TIMEOUT) as resp:
                    ...
                    if not stop_flag:
                        print(f"[{code}] {resp_time:.0f}ms - Total: {request_counter}")
                    success = True
                ...
                if retry == MAX_RETRY:
                    ...
                    if not stop_flag:
                        print(f"[{e.code}] HTTP Error setelah {MAX_RETRY}x retry")
                ...
                if retry == MAX_RETRY:
                    ...
                    if not stop_flag:
                        print(f"[TIMEOUT] Setelah {MAX_RETRY}x retry")
                ...
                if retry == MAX_RETRY:
                    with lock:
                        ...
                        error_codes['OTHER'] += 1
                    if not stop_flag:
                        print(f"[ERR] Setelah {MAX_RETRY}x retry")
...
def monitor(total_requests):
    ...
    while request_counter < total_requests and not stop_flag:
        ...
        print(f"Progress: {request_counter}/{total_requests} | OK: {ok} | Err: {error_counter} | {err_rate:.1f}%")
        time.sleep(2)
